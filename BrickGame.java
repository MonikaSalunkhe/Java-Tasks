...
class BreaksBreaker 
{
	...
	public BreaksBreaker(int row, int col)
	{
		map= new int[row][col];
...
class GamePlay extends JPanel implements KeyListener, ActionListener
{
	...
	private BreaksBreaker map;
	
	public GamePlay()
	{
		map= new BreaksBreaker(3,7);
		addKeyListener(this);
		setFocusable(true);
	...
	public void paint( Graphics g)
	{
		...
		g.drawString("You Won , Score:"+score,520,30);
		
		if(totalBricks<=0)
		{// if all bricks destroyed then you win
		play=false;
		ballXdir=0;
		ballYdir=0;
...
public void actionPerformed(ActionEvent arg0)
{
	...
	if(play)
	{
		...
		for(int i=0; i<map.map.length; i++)// ball bricks interaction
		{
		for(int j=0; i<map.map[0].length;j++)//map.map[0].length is the number of columns
		{
			if(map.map[i][j]>0)
			{
	...
	ballposY+= ballYdir;
	if(ballposX<0)
	{          // if ball hits left wall then it bounces back
	ballXdir=-ballXdir;
	}
	if(ballposY<0)
...
class BrickGame {
	public static void main(String args[])
	{
		...
		j1.setTitle("Brick Breaker Game");
